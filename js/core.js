...
function generateIPLinks(data, divtag) {
    var headersContainer = $('#' + divtag);
    data.headers.forEach(function (header) {
        var headerDiv = $('<div class="mb-5"></div>');
        ...
        var linkGroup = $('<div class="mb-1"></div>');
        header.links.forEach(function (link) {
            var ip = (link.port.length > 3) ? header.ip + ":" + link.port : header.ip;
            var anchor = $('<a href="' + link.scheme + "://" + ip + link.url + '" class="btn btn-primary" target="_blank">' + link.label + '</a>');
            linkGroup.append(anchor);
        });
